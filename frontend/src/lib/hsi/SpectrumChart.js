...
export default class SpectrumChart {
    constructor(canvas) {
        this.canvas = canvas;
        this.chart = null;
    ...
    renderSpectrum(wavelengths, values, label = 'Spectrum', color = 'rgb(147, 51, 234)', backgroundColor) {
        if (!this.chart) {
            this.initialize();
        }
        ...
        this.chart.data.datasets = [{
            ...
            data: values,
            borderColor: color,
            backgroundColor: backgroundColor || color.replace('rgb', 'rgba').replace(')', ', 0.1)'),
            borderWidth: 2,
            pointRadius: 0,
            pointHoverRadius: 4,
            tension: 0.1
    ...
    renderSeries(seriesList = []) {
        ...
        const baseLabels = seriesList[0].wavelengths.map((wl) => wl.toFixed(2));
        this.chart.data.labels = baseLabels;
        this.chart.data.datasets = seriesList.map((series) => ({
            label: series.label,
            data: series.values,
            borderColor: series.color || 'rgb(147, 51, 234)',
            backgroundColor: series.backgroundColor || 'rgba(147, 51, 234, 0.1)',
            borderWidth: 2,
            pointRadius: 0,
            pointHoverRadius: 4,
            tension: 0.1
